...
function TServerMethods1(connectionInfo)
{
  ...
  /*
   * @return result - Type on server: AnsiString
   */
  this.GetEmployeeList = function() {
    var returnObject = this.executor.executeMethod('GetEmployeeList', "GET", [], arguments[0], true, arguments[1], arguments[2]);
    if (arguments[0] == null) {
      if (returnObject != null && returnObject.result != null && isArray(returnObject.result)) {
        var resultArray = returnObject.result;
        var resultObject = new Object();
  ...
  this.GetEmployeeList_URL = function() {
    return this.executor.getMethodURL("GetEmployeeList", "GET", [], arguments[0])[0];
  };

  /*
   * @param EmpName [in] - Type on server: AnsiString
   * @param Password [in] - Type on server: AnsiString
   * @return result - Type on server: Boolean
   */
  this.ValidateLogin = function(EmpName, Password) {
    var returnObject = this.executor.executeMethod('ValidateLogin', "GET", [EmpName, Password], arguments[2], true, arguments[3], arguments[4]);
    if (arguments[2] == null) {
      if (returnObject != null && returnObject.result != null && isArray(returnObject.result)) {
        var resultArray = returnObject.result;
        var resultObject = new Object();
        resultObject.EmpName = EmpName;
  ...
  this.ValidateLogin_URL = function(EmpName, Password) {
    return this.executor.getMethodURL("ValidateLogin", "GET", [EmpName, Password], arguments[2])[0];
  };

  /*
   * @param reqDate [in] - Type on server: AnsiString
   * @param SelEmployee [in] - Type on server: AnsiString
   * @return result - Type on server: AnsiString
   */
  this.GetTimes = function(reqDate, SelEmployee) {
    var returnObject = this.executor.executeMethod('GetTimes', "GET", [reqDate, SelEmployee], arguments[2], true, arguments[3], arguments[4]);
    if (arguments[2] == null) {
      if (returnObject != null && returnObject.result != null && isArray(returnObject.result)) {
        var resultArray = returnObject.result;
        var resultObject = new Object();
        resultObject.reqDate = reqDate;
        resultObject.SelEmployee = SelEmployee;
        resultObject.result = resultArray[0];
        return resultObject;
      }
      return returnObject;
  ...
  this.GetTimes_URL = function(reqDate, SelEmployee) {
    return this.executor.getMethodURL("GetTimes", "GET", [reqDate, SelEmployee], arguments[2])[0];
  };

  /*
   * @return result - Type on server: AnsiString
   */
  this.GetPayrollItems = function() {
    var returnObject = this.executor.executeMethod('GetPayrollItems', "GET", [], arguments[0], true, arguments[1], arguments[2]);
    if (arguments[0] == null) {
      if (returnObject != null && returnObject.result != null && isArray(returnObject.result)) {
        var resultArray = returnObject.result;
        var resultObject = new Object();
  ...
  this.GetPayrollItems_URL = function() {
    return this.executor.getMethodURL("GetPayrollItems", "GET", [], arguments[0])[0];
  };

  /*
   * @param reqDateI [in] - Type on server: AnsiString
   * @param Employee [in] - Type on server: AnsiString
   * @param PayrollItem [in] - Type on server: AnsiString
   * @param StartTime [in] - Type on server: AnsiString
   * @param EndTime [in] - Type on server: AnsiString
   * @return result - Type on server: AnsiString
   */
  this.ValidateTimeEntry = function(reqDateI, Employee, PayrollItem, StartTime, EndTime) {
    var returnObject = this.executor.executeMethod('ValidateTimeEntry', "GET", [reqDateI, Employee, PayrollItem, StartTime, EndTime], arguments[5], true, arguments[6], arguments[7]);
    if (arguments[5] == null) {
      if (returnObject != null && returnObject.result != null && isArray(returnObject.result)) {
        var resultArray = returnObject.result;
        var resultObject = new Object();
        resultObject.reqDateI = reqDateI;
        resultObject.Employee = Employee;
        resultObject.PayrollItem = PayrollItem;
  ...
  this.ValidateTimeEntry_URL = function(reqDateI, Employee, PayrollItem, StartTime, EndTime) {
    return this.executor.getMethodURL("ValidateTimeEntry", "GET", [reqDateI, Employee, PayrollItem, StartTime, EndTime], arguments[5])[0];
  };

  /*
   * @param Direction [in] - Type on server: Integer
   * @return result - Type on server: AnsiString
   */
  this.GetPeriod = function(Direction) {
    var returnObject = this.executor.executeMethod('GetPeriod', "GET", [Direction], arguments[1], true, arguments[2], arguments[3]);
    if (arguments[1] == null) {
      if (returnObject != null && returnObject.result != null && isArray(returnObject.result)) {
        var resultArray = returnObject.result;
        var resultObject = new Object();
  ...
  this.GetPeriod_URL = function(Direction) {
    return this.executor.getMethodURL("GetPeriod", "GET", [Direction], arguments[1])[0];
  };

  /*
   * @param EmpName [in] - Type on server: AnsiString
   * @param Period [in] - Type on server: AnsiString
   * @return result - Type on server: AnsiString
   */
  this.GetTimeSheet = function(EmpName, Period) {
    var returnObject = this.executor.executeMethod('GetTimeSheet', "GET", [EmpName, Period], arguments[2], true, arguments[3], arguments[4]);
    if (arguments[2] == null) {
      if (returnObject != null && returnObject.result != null && isArray(returnObject.result)) {
        var resultArray = returnObject.result;
        var resultObject = new Object();
  ...
  this.GetTimeSheet_URL = function(EmpName, Period) {
    return this.executor.getMethodURL("GetTimeSheet", "GET", [EmpName, Period], arguments[2])[0];
  };
}
...
var JSProxyClassList = {
  ...
  "TServerMethods1": ["GetEmployeeList","ValidateLogin","GetTimes","GetPayrollItems","ValidateTimeEntry","GetPeriod","GetTimeSheet"]
};
